...
const Home = () => {
  const activeUser = useSelector((s) => s.auth.user)
  return (
    <div className="bg-blue-700 w-1/6 pb-6 hidden md:block">
      <h1 className="text-white text-xl mb-2 mt-3 px-4 font-sans flex justify-between">
        <span>pauchok chat</span>
        <svg className="h-6 w-6 text-purple-600 fill-current" viewBox="0 0 32 32">
          <g id="surface1">
      ...
      <div className="flex items-center mb-6 px-4">
        ...
        <span className="text-purple-lightest">{activeUser.email}</span>
      </div>
